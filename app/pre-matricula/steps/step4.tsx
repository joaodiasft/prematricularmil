...
import { Calendar, Clock, GraduationCap, Check, ArrowRight, ArrowLeft } from "lucide-react"
...
export default function Step4({ onNext, onBack }: Step4Props) {
  ...
  const [selectedClasses, setSelectedClasses] = useState<Record<string, string>>({}) // { subjectType: classId }
  const [classes, setClasses] = useState<{ highSchool: Class[]; middleSchool: Class[] }>({
    highSchool: [],
    middleSchool: [],
  })
  ...
  const handleClassSelect = (classItem: Class) => {
    ...
    localStorage.setItem("preMatricula_step4", JSON.stringify({ selectedClasses: newSelected }))
  }
  ...
  const getAvailabilityStatus = (classItem: Class) => {
    ...
    if (available === 0) return { label: "Lista de Espera", color: "bg-gray-500" }
    if (available <= 5) return { label: "Poucas Vagas", color: "bg-orange-500" }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (selectedCount === 0) {
      toast({
        ...
        description: "Você pode selecionar uma turma por matéria. Selecione pelo menos uma para continuar.",
        variant: "destructive",
      })
      return
    }
  ...
  const renderClassCard = (classItem: Class) => {
    ...
    const isSelected = getSelectedClassForSubject(classItem.subject.type) === classItem.id
        ...
        <CardHeader>
          <div className="flex items-start justify-between">
            <div className="flex-1">
              ...
              <CardDescription className="mt-1">{classItem.code} - {classItem.subject.name}</CardDescription>
            </div>
            {isSelected && (
      ...
      return (
        <div key={subjectType} className="space-y-3">
          <div className="flex items-center justify-between">
            <h4 className="font-semibold text-lg">{subjectNames[subjectType] || subjectType}</h4>
            {hasSelection && (
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Selecione uma turma por matéria. Você pode escolher Redação, Matemática, Exatas, etc. É obrigatório selecionar pelo menos uma turma.
        </CardDescription>
          ...
          <Tabs defaultValue="highSchool" className="w-full">
            ...
            <TabsContent value="highSchool" className="space-y-6">
              {loading ? (
                ...
                <div className="text-center py-8 text-gray-500">
                  <p className="mb-2">Nenhuma turma disponível para Ensino Médio</p>
                  <Button 
                    variant="outline" 
                    size="sm"
                    onClick={() => {
            ...
            <TabsContent value="middleSchool" className="space-y-6">
              {loading ? (
                ...
                <div className="text-center py-8 text-gray-500">
                  <p className="mb-2">Nenhuma turma disponível para Ensino Fundamental</p>
                  <Button 
                    variant="outline" 
                    size="sm"
                    onClick={() => {
